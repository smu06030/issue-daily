...
const TopNewsCard = ({ post }: { post: NewsResultsType }) => {
  return (
    <Link href={`/detail/${post.article_id}`} className="flex embla__slide grow-0 shrink-0 basis-full w-full h-full">
      ...
      <div className="w-[50%] mx-5">
        ...
        <p className="break-keep text-ellipsis ">{post.description === null ? post.link : post.description}</p>
      </div>
    </Link>
  );
};
